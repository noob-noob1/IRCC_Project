...
def handle_missing_data(df):
    ...
    if columns_to_drop:
        ...
        logging.info(f"Dropped columns with >90% missing values: {', '.join(columns_to_drop)}")
    else:
    ...
    if final_missing > 0:
         logging.warning("Some NaNs might remain in non-numeric columns or if means couldn't be calculated.")
...
def perform_feature_engineering(df):
    ...
    if "Total, work status" in df_fe.columns and "Total operating expenditures" in df_fe.columns:
        df_fe["Educator_to_OperatingSpending"] = df_fe["Total, work status"] / df_fe["Total operating expenditures"]
        logging.info("Created feature: Educator_to_OperatingSpending")
    else:
        logging.warning("Skipping Educator_to_OperatingSpending: Required columns missing.")

    # 2. Salary-to-EPI ratio
    if "Teachers salaries" in df_fe.columns and "Education price index (EPI)" in df_fe.columns:
        df_fe["Salary_to_EPI"] = df_fe["Teachers salaries"] / df_fe["Education price index (EPI)"]
    ...
    if "Total operating expenditures" in df_fe.columns and "Total, work status" in df_fe.columns:
        df_fe["OpSpend_per_Educator"] = df_fe["Total operating expenditures"] / df_fe["Total, work status"]
    ...
    available_participation_cols = [col for col in participation_cols if col in df_fe.columns]
    if len(available_participation_cols) > 0:
        df_fe["Education_Access_Index"] = df_fe[available_participation_cols].mean(axis=1, skipna=True)
        logging.info(f"Created feature: Education_Access_Index using columns: {', '.join(available_participation_cols)}")
    else:
        logging.warning("Skipping Education_Access_Index: No participation columns found.")

    # 5. Capital efficiency
    if "Teachers' salaries sub-index" in df_fe.columns and "Total expenditures" in df_fe.columns:
        df_fe["Capital_Efficiency"] = df_fe["Total expenditures"] / df_fe["Teachers' salaries sub-index"]
    ...
    new_feature_cols = [
        "Educator_to_OperatingSpending", "Salary_to_EPI", "OpSpend_per_Educator",
        "Education_Access_Index", "Capital_Efficiency"
    ]
...
def enhance_education_data(input_path, output_path):
    ...
    try:
        ...
        logging.error(f"An unexpected error occurred during the education enhancement pipeline: {e}")

if __name__ == '__main__':
    # Example usage: Define paths relative to the project root
    input_file = os.path.join('data/processed', 'Education_Features_Merged.csv')
    output_file = os.path.join('data/processed', 'Education_Features_Enhanced.csv')

    enhance_education_data(input_file, output_file)
