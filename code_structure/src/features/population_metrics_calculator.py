...
def calculate_population_metrics(df):
    ...
    for col in numeric_cols:
        if col in df.columns:
            ...
            if na_count > 0:
                logging.warning(
                    f"Column '{col}': {na_count} NaN values introduced after converting to numeric."
                )
        else:
            logging.warning(
                f"Column '{col}' not found. Filling with 0 for metric calculation."
            )
            df[col] = 0
    ...
    logging.debug(
        f"Sample of 'Natural Increase': {df['Natural Increase'].head().tolist()}"
    )
    ...
    logging.debug(
        f"Sample of 'Net Population Change': {df['Net Population Change'].head().tolist()}"
    )

    # Calculate Rates (per 1000 for migration/natural growth, % for overall growth)
    logging.debug("Calculating rates.")
    df["Net Migration Rate"] = np.divide(
        df["Net Migration"] * 1000,
        df["Population Estimate"],
        out=np.zeros_like(df["Net Migration"] * 1000, dtype=float),
        where=df["Population Estimate"] != 0,
    )
    df["Natural Growth Rate"] = np.divide(
        df["Natural Increase"] * 1000,
        df["Population Estimate"],
        out=np.zeros_like(df["Natural Increase"] * 1000, dtype=float),
        where=df["Population Estimate"] != 0,
    )
    df["Population Growth Rate (%)"] = np.divide(
        df["Net Population Change"] * 100,
        df["Population Estimate"],
        out=np.zeros_like(df["Net Population Change"] * 100, dtype=float),
        where=df["Population Estimate"] != 0,
    )
    logging.debug(
        "Calculated 'Net Migration Rate', 'Natural Growth Rate', and 'Population Growth Rate (%)'."
    )
    logging.debug(
        f"Sample of 'Net Migration Rate': {df['Net Migration Rate'].head().tolist()}"
    )
    logging.debug(
        f"Sample of 'Natural Growth Rate': {df['Natural Growth Rate'].head().tolist()}"
    )
    logging.debug(
        f"Sample of 'Population Growth Rate (%)': {df['Population Growth Rate (%)'].head().tolist()}"
    )
    ...
    for col in rate_cols:
        ...
        if na_count_before_fill > 0:
            logging.warning(
                f"Column '{col}': Filling {na_count_before_fill} NaN values with 0."
            )
        df[col] = df[col].fillna(0)
        na_count_after_fill = df[col].isna().sum()
        if na_count_after_fill > 0:
            logging.error(
                f"Column '{col}': Still has {na_count_after_fill} NaN values after fillna(0). Check for issues."
            )
